...
namespace LSMKV {
  ...
  static inline void CreateFilter(Slice *keys, size_t n, std::string *dst) {
      ...
      for (int i = 0; i < n; i++) {
          auto slice = keys[i];
          MurmurHash3_x64_128(slice.data(), slice.size(), 1,
  ...
  static inline void CreateFilter(const char *keys, size_t n, int len, char *dst) {
      ...
      for (int i = 0; i < n; i++) {
          MurmurHash3_x64_128(keys + len * i, 8, 1,
                              reinterpret_cast<char *>(hash.data()));
          for (uint32_t h: hash) {
              if constexpr (use_double_k) {
                  const uint32_t delta = (h >> 17) | (h << 15);
  ...
  static inline bool KeyMayMatch(const Slice &key, const Slice &bloom_filter) {
      ...
      std::array<uint32_t, 4> hash{};
      MurmurHash3_x64_128(key.data(), key.size(), 1,
                          reinterpret_cast<char *>( hash.data()));

      return std::all_of(hash.begin(), hash.end(), [bits = len * 8, array](uint32_t h) {
          if constexpr (use_double_k) {
              const uint32_t delta = (h >> 17) | (h << 15);
